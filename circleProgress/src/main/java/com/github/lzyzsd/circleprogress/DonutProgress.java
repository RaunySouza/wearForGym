...
public class DonutProgress extends View {
    ...
    private static final String INSTANCE_ANGLE = "angle";
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        angle = attributes.getFloat(R.styleable.DonutProgress_donut_angle, 0f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(finishedOuterRect, angle, getProgressAngle(), false, finishedPaint);
        canvas.drawArc(unfinishedOuterRect, getProgressAngle() + angle, 360 - getProgressAngle(), false, unfinishedPaint);

        String text = this.text != null ? this.text : prefixText + progress + suffixText;
        if (!TextUtils.isEmpty(text) && showText) {
            float textHeight = textPaint.descent() + textPaint.ascent();
            canvas.drawText(text, (getWidth() - textPaint.measureText(text)) / 2.0f, (getWidth() - textHeight) / 2.0f, textPaint);
        }

        if (!TextUtils.isEmpty(getInnerBottomText()) && showText) {
            innerBottomTextPaint.setTextSize(innerBottomTextSize);
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        bundle.putBoolean(INSTANCE_SHOW_TEXT, isShowText());
        ...
        bundle.putFloat(INSTANCE_ANGLE, angle);
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if(state instanceof Bundle) {
            ...
            initPainters();
            angle = bundle.getFloat(INSTANCE_ANGLE);
